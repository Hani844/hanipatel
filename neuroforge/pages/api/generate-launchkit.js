...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [website, video] = await Promise.all([generateWebsite(prompt), generateVideo(prompt)]);
    const websiteZip = await createWebsiteZip(website);
    const usage = incrementUsage(user.id);

    addHistory(user.id, { type: 'launchkit', prompt, summary: website.layout?.hero?.title || 'Launch kit generation' });
    ...
    return res.status(200).json({
      data: {
        ...
        video,
        websiteZip,
        previewUrl: `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/preview/launchkit/${Date.now()}`,
        usage,
    ...
    return res.status(400).json({ error: error.message || 'Failed to generate launch kit' });
  }
};
